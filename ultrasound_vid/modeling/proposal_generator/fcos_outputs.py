...
from ultrasound_vid.utils.comm import reduce_sum
from ultrasound_vid.modeling.losses import IOULoss
...
class FCOSOutputs(nn.Module):
    ...
    def forward_for_single_feature_map(
        ...
        logits_pred = logits_pred.view(N, C, H, W).permute(0, 2, 3, 1).contiguous()
        logits_pred = logits_pred.reshape(N, -1, C).sigmoid()
        box_regression = reg_pred.view(N, 4, H, W).permute(0, 2, 3, 1).contiguous()
        box_regression = box_regression.reshape(N, -1, 4)
        ctrness_pred = ctrness_pred.view(N, 1, H, W).permute(0, 2, 3, 1).contiguous()
        ctrness_pred = ctrness_pred.reshape(N, -1).sigmoid()
        if top_feat is not None:
            top_feat = top_feat.view(N, -1, H, W).permute(0, 2, 3, 1).contiguous()
