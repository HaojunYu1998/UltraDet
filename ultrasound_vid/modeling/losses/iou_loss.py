...
def iou_loss(
    ...
    w_intersect = (
        torch.min(inputs[..., 2], targets[..., 2])
        - torch.max(inputs[..., 0], targets[..., 0])
    ).clamp_(min=0)
    h_intersect = (
        torch.min(inputs[..., 3], targets[..., 3])
        - torch.max(inputs[..., 1], targets[..., 1])
    ).clamp_(min=0)
    ...
    if loss_type == "iou":
        ...
        g_w_intersect = torch.max(inputs[..., 2], targets[..., 2]) - torch.min(
            inputs[..., 0], targets[..., 0]
        )
        g_h_intersect = torch.max(inputs[..., 3], targets[..., 3]) - torch.min(
            inputs[..., 1], targets[..., 1]
        )
