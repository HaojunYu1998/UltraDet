...
frame_cache = namedtuple("frame_cache", ["tgt", "query_embed", "memory", "lvl_embed", "valid_ratio", "mask"])
...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        self.temperature = pos_temperature
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, return_intermediate=False, encoder_only=False, images=None):
        ...
        if encoder_only:
            return query_embed, memory, spatial_shapes, level_start_index, valid_ratios, mask_flatten
        ...
        if not self.training and self.temporal_attn:
            tgt, query_embed, memory, lvl_pos_embed_flatten, valid_ratios, mask_flatten = self.concat_temporal(
                tgt, query_embed, memory, lvl_pos_embed_flatten, valid_ratios, mask_flatten
            )
        ...
        if return_intermediate: # return intermediate results for temporal transformer
            if self.two_stage:
                return (
                    hs, init_reference_out, inter_references_out, enc_outputs_class, enc_outputs_coord_unact,
                    query_embed, memory, lvl_pos_embed_flatten, spatial_shapes, level_start_index, valid_ratios, mask_flatten
                )
            return (
    ...
    @torch.no_grad()
    def concat_temporal(self, tgt, query_embed, memory, lvl_pos_embed, valid_ratio, mask):
        """
        Params:
            tgt: (bs, nq, d_model)
        ...
        self.history_buffer.append(
            frame_cache(tgt, query_embed, memory, lvl_pos_embed, valid_ratio, mask)
        )
        ...
        return tgt, query_embed, memory, lvl_pos_embed, valid_ratio, mask
