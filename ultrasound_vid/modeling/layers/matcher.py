...
from ultrasound_vid.utils.box_ops import box_cxcywh_to_xyxy, generalized_box_iou
...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        with torch.no_grad():
            ...
            assert (bs == 1) or (bs == len(targets)), f"{bs} != 1 and {bs} != {len(targets)}"
            targets = [targets[i] for i in range(bs)]
