...
class FlowNetS(nn.Module):
    def __init__(self, cfg, flownet_method=""):
        super(FlowNetS, self).__init__()
        self.method = cfg.MODEL.FLOWNET_METHOD
        if len(flownet_method) > 0:
            self.method = flownet_method
        self.pool_stride = cfg.MODEL.FLOWNET_POOL_STRIDE
        assert self.method in ["FGFA", "DFF"]
        ...
        if self.method == "DFF":
            self.Convolution5_scale = nn.Conv2d(194, 256, kernel_size=1, stride=1, padding=0, bias=False)
        ...
        self.avgpool = nn.AvgPool2d(self.pool_stride, stride=self.pool_stride, ceil_mode=True)
