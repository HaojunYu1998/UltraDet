...
def inference_on_video_dataset(
    model, data_loader, dataset_name, save_folder=None, rpn_only=False, skip_exists=False
):
    """
    ...
    logger.info("{} videos are allocated to this device.".format(len(data_loader)))

    total_videos = len(data_loader)  # inference data loader must have a fixed length
    evaluator = UltrasoundVideoDetectionEvaluator(dataset_name, rpn_only)
    ...
    with video_inference_context(model), torch.no_grad():
        for i_video, (video_annotation, video_reader) in enumerate(data_loader):
            ...
            for i_frame, frame in enumerate(video_reader):
                ...
                eta = datetime.timedelta(
                    seconds=int(1 / frames_per_second * (total_frames_i - i_frame - 1))
                )
                log_every_n_seconds(
                    logging.INFO,
                    "Video {}/{}. Inference done {:5d}/{:5d} frames. {:2.2f} frames/s. ETA={}".format(
                        i_video + 1,
                        total_videos,
                        i_frame + 1,
                        total_frames_i,
            ...
            logger.info(
                "Video {}/{} finished. ({:.2f} frames / s)".format(
    ...
    logger.info(
        "Total inference time: {} ({:.2f} frames / s per device, on {} devices)".format(
            total_time_str, total_frames / total_time, num_devices
        )
    )
    total_compute_time_str = str(datetime.timedelta(seconds=int(total_compute_time)))
    logger.info(
        "Total inference pure compute time: {} ({:.2f} frames / s per device, on {} devices)".format(
            total_compute_time_str, total_frames / total_compute_time, num_devices
        )
    )
