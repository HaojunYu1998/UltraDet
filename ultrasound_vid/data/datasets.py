...
def load_ultrasound_annotations(coco_anno, image_root, dataset_name):
...
def build_bus_dataset(suffix, split):
    BUS_IMAGE_ROOT = (PROJ_ROOT / "datasets" / f"bus_data{suffix}" /
                      "rawframes").realpath()
    BUS_JSON_FILE = (PROJ_ROOT / "datasets" / f"bus_data{suffix}" /
                     f"{split}.json").realpath()
